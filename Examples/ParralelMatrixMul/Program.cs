...
const int TREADS_NUMBER = 10;

int[,] serialMulRes = new int[N, N]; // Результат умножения в однопотоке
int[,] threadMulRes = new int[N, N]; // Результат умножение в многопотоке
...
{
  if (a.GetLength(1) != b.GetLength(0)) throw new Exception("Нельзя уножать такие матрицы!");
...
void PrepareParallelMatrixMul(int[,] a, int[,] b)
{
  ...
  int eachThreadCalc = N / TREADS_NUMBER;
